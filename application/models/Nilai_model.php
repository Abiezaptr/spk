...
class Nilai_model extends CI_Model
{
    ...
    public function get_AllGuru($tahun)
    {
        ...
        $query = "SELECT `id_nilai`, `nama_karyawan`, `nip`, p.id_karyawan 
        FROM `karyawan` g 
        join `nilai` p on g.id_karyawan = p.id_karyawan WHERE p.id_periode = ?  GROUP BY p.id_karyawan";
    ...
    {
        // return $this->db->get_where('nilai', ['id_periode' => $tahun])->result_array();
        $query = "SELECT `id_nilai`, `nama_karyawan`, `nip`, p.id_karyawan FROM `karyawan` g join 
        `nilai` p on g.id_karyawan = p.id_karyawan WHERE p.id_periode = ? AND id_admin = ?  GROUP BY p.id_karyawan";
    ...
    {
        // return $this->db->get_where('nilai', ['id_periode' => $tahun])->result_array();
        $query = "SELECT `id_nilai`, `nama_karyawan`, `jabatan`, `nip`, p.id_karyawan, p.nilai_karyawan FROM `karyawan` g join 
        `nilai` p on g.id_karyawan = p.id_karyawan WHERE p.id_periode = ? AND p.id_karyawan = ?";
    ...
    public function Edit($id, $tahun)
    {
        ...
        $C1 = "UPDATE nilai SET nilai_karyawan = ? WHERE id_kriteria = 1 AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($C1, array($nilaiC1, $id, $tahun));

        $nilaiC2 = $this->input->post('C2');
        $C2 = "UPDATE nilai SET nilai_karyawan = ? WHERE id_kriteria = 2 AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($C2, array($nilaiC2, $id, $tahun));

        $nilaiC3 = $this->input->post('C3');
        $C3 = "UPDATE nilai SET nilai_karyawan = ? WHERE id_kriteria = 3 AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($C3, array($nilaiC3, $id, $tahun));

        $nilaiC4 = $this->input->post('C4');
        $C4 = "UPDATE nilai SET nilai_karyawan = ? WHERE id_kriteria = 4 AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($C4, array($nilaiC4, $id, $tahun));

        $nilaiC5 = $this->input->post('C5');
        $C5 = "UPDATE nilai SET nilai_karyawan = ? WHERE id_kriteria = 5 AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($C5, array($nilaiC5, $id, $tahun));

        $nilaiC6 = $this->input->post('C6');
        $C6 = "UPDATE nilai SET nilai_karyawan = ? WHERE id_kriteria = 6 AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($C6, array($nilaiC6, $id, $tahun));

        $nilaiC7 = $this->input->post('C7');
        $C7 = "UPDATE nilai SET nilai_karyawan = ? WHERE id_kriteria = 7 AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($C7, array($nilaiC7, $id, $tahun));

        $nilaiC8 = $this->input->post('C8');
        $C8 = "UPDATE nilai SET nilai_karyawan = ? WHERE id_kriteria = 8 AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($C8, array($nilaiC8, $id, $tahun));

        $nilaiC9 = $this->input->post('C9');
        $C9 = "UPDATE nilai SET nilai_karyawan = ? WHERE id_kriteria = 9 AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($C9, array($nilaiC9, $id, $tahun));

        $nilaiC10 = $this->input->post('C10');
        $C10 = "UPDATE nilai SET nilai_karyawan = ? WHERE id_kriteria = 10 AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($C10, array($nilaiC10, $id, $tahun));

        $nilaiC11 = $this->input->post('C11');
        $C11 = "UPDATE nilai SET nilai_karyawan = ? WHERE id_kriteria = 11 AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($C11, array($nilaiC11, $id, $tahun));

        $nilaiC12 = $this->input->post('C12');
        $C12 = "UPDATE nilai SET nilai_karyawan = ? WHERE id_kriteria = 12 AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($C12, array($nilaiC12, $id, $tahun));

        $nilaiC13 = $this->input->post('C13');
        $C13 = "UPDATE nilai SET nilai_karyawan = ? WHERE id_kriteria = 13 AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($C13, array($nilaiC13, $id, $tahun));

        $nilaiC14 = $this->input->post('C14');
        $C14 = "UPDATE nilai SET nilai_karyawan = ? WHERE id_kriteria = 14 AND id_karyawan = ? AND id_periode = ?";
        $this->db->query($C14, array($nilaiC14, $id, $tahun));
    }

    public function get_NilaiGuru($id, $tahun)
    {
        $query = "SELECT * FROM nilai WHERE id_karyawan = ? AND id_periode = ?";
    ...
    {
        $query = "SELECT `id_penilaian`, p.id_nilai, n.id_karyawan, p.nilai, p.normalisasi, p.terbobot FROM `nilai` n join 
        `penilaian` p on n.id_nilai = p.id_nilai WHERE n.id_karyawan = ? AND n.id_periode = ? ORDER BY id_penilaian";
        return $this->db->query($query, array($id, $tahun))->result_array();
    }
    ...
    {
        $query = "SELECT id_nilai, id_karyawan, k.id_kriteria, bobot, jenis FROM nilai n join
    ...
    public function select_nilai($id, $tahun, $id_a)
    {
        ...
        where id_karyawan = ? and id_periode = ? and p.id_alternatif = ?";
    ...
    public function get_hasil($id, $tahun)
    {
        ...
        alternatif a on p.id_alternatif = a.id_alternatif WHERE n.id_karyawan = ? AND n.id_periode = ? ORDER BY nilai_akhir DESC";
    ...
    {
        $query = "UPDATE penilaian p join nilai n on n.id_nilai = p.id_nilai set rank = ? WHERE id_karyawan = ? AND id_periode = ?";
